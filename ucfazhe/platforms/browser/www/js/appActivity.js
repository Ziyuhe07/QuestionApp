...
var geoJSONString;
	geoJSONString = getJSON('http://developer.cege.ucl.ac.uk:30271/getGeoJSON/questionform/geom');
...
function getDistanceFromPoint(position) {
		 ...
		 // these are the coordinates for Euston Square
		 var coordinates = geoJSON[0].features[i];
		 for (component in coordinates){
		 	if (component =="geometry"){
		 		for (geometry in coordinates[component]){
			     var lng=coordinates[component][geometry][0];
				 var lat=coordinates[component][geometry][1];
				 

		 // return the distance in kilometers
		 var distance = calculateDistance(position.coords.latitude, position.coords.longitude, lat,lng, 'K');
		 document.getElementById('getDistance').innerHTML = "Distance: " + distance;
		 if (distance<0.16){
		 	L.marker([lat,lng]).addTo(mymap) //Marker on the downloaded data
		 	.bindPopup("<b>You were at "+ lng + " "+lat+"!</b>");
		 	mymap.setView([lng, lat], 13);
		 }
}
}
}
